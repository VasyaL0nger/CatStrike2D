...
def save_users(db):
    json.dump(db, open(USERS_FILE, "w"))
...
# ================= РЕЖИМ 1: РЕГИСТРАЦИЯ И ВХОД =================
if not st.session_state.logged_user:
    st.title("🐱 CatStrike 2D: Авторизация 🔐")
    menu_auth = st.radio("Выберите действие:", ["Войти в аккаунт", "Создать новый аккаунт"], horizontal=True)
    
    username = st.text_input("Введите ваш логин (английскими буквами):").strip().lower()
    password = st.text_input("Введите ваш пароль:", type="password").strip()
    
    db = load_users()
    
    if menu_auth == "Создать новый аккаунт":
        if st.button("🆕 ЗАРЕГИСТРИРОВАТЬСЯ", use_container_width=True):
            if not username or not password:
                st.error("Поля не могут быть пустыми!")
            elif username in db:
                st.error("Такой логин уже занят другим боевым котом!")
            else:
                db[username] = {"password": password, "food": 100, "current_rank": "начальный"}
                save_users(db)
                st.success("Аккаунт успешно создан! Теперь переключитесь на 'Войти в аккаунт'.")
                
    else:
        if st.button("🔓 ВОЙТИ В ШТАБ", use_container_width=True):
            if username in db and db[username]["password"] == password:
                st.session_state.logged_user = username
                st.session_state.food = db[username]["food"]
                st.session_state.current_rank = db[username]["current_rank"]
                st.rerun()
            else:
                st.error("Неверный логин или пароль!")
    st.stop()

# --- ЕСЛИ ИГРОК ВОШЕЛ — ПОДГРУЖАЕМ ЕГО ДАННЫЕ ---
...
# Панель профиля в сайдбаре
st.sidebar.markdown(f"👤 Игрок: **{user.upper()}**")
...
with tab_g:
    # ================= РЕЖИМ 2: ШТАБ-КВАРТИРА =================
    if not st.session_state.match_playing:
        st.title("🐾 CatStrike 2D: Главный Штаб")
        
        # ИСПРАВЛЕНО: Добавили цифру 2 внутрь скобок columns(2), ошибка стёрта!
        c_room, c_desc = st.columns(2)
        with c_room:
            st.markdown("""
            <style>
        ...
        with c_desc:
            ...
            if cat == "v":
                ...
                st.write("🐾 **Буля**: Очень нежная белая кошка.")
                if st.button("🧼 Погладить"): st.info("Буля громко мурлычет!")
            elif cat == "m": st.warning("🥷 **Мурка**: Царапается и прыгает по диванам!")
            elif cat == "r": st.write("🦁 **Рыжик**: Рыжий, быстрый и смертельно опасный.")
            elif cat == "t": st.write("🐯 **Томас**: Полосатый лучший друг Рыжика.")
            else: st.info("Кликни кота на базе слева!")

        st.write("---")
        chosen = st.selectbox("Выбери бойца в рейд:", ["Vasya", "Bulya", "Murka", "Rizyk", "Tomas", "ADMIN"])
        if st.button("⚔️ НАЧАТЬ РЕЙД НА АРЕНЕ", use_container_width=True):
            st.session_state.match_playing = True
            st.session_state.chosen_hero = chosen
            st.query_params.clear()
            st.rerun()

    # ================= РЕЖИМ 3: ИГРОВАЯ АРЕНА =================
    else:
...
Используйте код с осторожностью.ctx.font=p.size+"px Arial"; ctx.fillText(p.emoji, p.x, p.y);bullets.forEach((b,idx)=>{{ b.x+=b.speed; ctx.beginPath(); ctx.arc(b.x,b.y,5,0,Math.PI2); ctx.fillStyle="#22c55e"; ctx.fill(); if(b.x>canvas.width)bullets.splice(idx,1); }});if(Math.random()<0.025) enemies.push({{x:canvas.width, y:Math.random()(canvas.height-50)+10, speed:Math.random()*1.5+2+speedBonus}});enemies.forEach((e,eIdx)=>{{ e.x-=e.speed; ctx.font="28px Arial"; ctx.fillText("🐀",e.x,e.y);bullets.forEach((b,bIdx)=>{{ if(b.x>e.x && b.x<e.x+30 && b.y>e.y && b.y<e.y+30){{ bullets.splice(bIdx,1); enemies.splice(eIdx,1); score+=10; if(score>=500) finish("win"); }} }});if(e.x<p.x+25 && e.x+25>p.x && e.y<p.y+25 && e.y+25>p.y){{ enemies.splice(eIdx,1); p.hp-=20; if(p.hp<=0) finish("lose"); }}if(e.x<-30) enemies.splice(eIdx,1);}});ctx.fillStyle="white"; ctx.font="16px Arial"; ctx.fillText(Кот: ${{p.name}} | ❤️ HP: ${{p.hp}}/${{p.maxHp}} | 🎯 Очки: ${{score}}/500,15,25);}}loop();"""st.components.v1.html(game_html, height=420)with tab_p:st.header("👤 Сетка твоих званий")for r_n, r_c in RANKS.items():is_curr = " (Текущий)" if st.session_state.current_rank == r_n else ""st.write(f"• {r_n.upper()} — требуется {r_c} еды {is_curr}")if st.sidebar.button("🧪 Читы: +5000 еды"):st.session_state.food += 5000db[user]["food"] = st.session_state.foodsave_users(db)st.rerun()
