    ...
    if not st.session_state.play:
        ...
        <!DOCTYPE html><html><head><style>
            ...
            <script>
                ...
                let currentWIdx = 0; let curW = myWeapons[currentWIdx]; let shootCount = 0;

                window.addEventListener("keydown",e=>{ if(play){ if(e.code==='Space'&&!keys['Space']&&t<=0){shoot();} if(e.code==='KeyQ') { switchWeapon(); } keys[e.code]=true; } });
                window.addEventListener("keyup",e=>{keys[e.code]=false;});
                canvas.addEventListener("mousedown",()=>{ if(play && t<=0 && !mOn) shoot(); });
                
                function switchWeapon() { if(myWeapons.length <= 1) return; currentWIdx = (currentWIdx + 1) % myWeapons.length; curW = myWeapons[currentWIdx]; }
                
                if(mOn) {
                    canvas.addEventListener("touchstart",e=>{ e.preventDefault(); for(let i=0;i<e.changedTouches.length;i++){ let tObj=e.changedTouches[i], r=canvas.getBoundingClientRect(), tx=tObj.clientX-r.left, ty=tObj.clientY-r.top; if(tx<canvas.width/2&&!joystickActive){ if(ty < 60) { switchWeapon(); } else { joystickActive=true; jId=tObj.identifier; joyCenter.x=tx; joyCenter.y=ty; joyStick.x=tx; joyStick.y=ty; } } else if(tx>=canvas.width/2&&t<=0&&play){shoot();} } });
                ...
                function shoot() { 
                    let bx=p1.x+20, by=p1.y+10, bid=1; if(!solo&&myId==2){ bx=p2.x+20; by=p2.y+10; bid=2; } 
                    shootCount++;
                    if(curW === "пистолет") { b.push({x:bx, y:by, id:bid, type:"normal"}); if(shootCount % 2 === 0) { b.push({x:bx+15, y:by-5, id:bid, type:"normal"}); } } 
                    else if(curW === "автомат") { b.push({x:bx, y:by, id:bid, type:"normal"}); if(shootCount % 2 === 0) { b.push({x:bx+10, y:by-15, id:bid, type:"normal", vy:-1.5}); b.push({x:bx+10, y:by+15, id:bid, type:"normal", vy:1.5}); b.push({x:bx+15, y:by, id:bid, type:"normal"}); } } 
                    else if(curW === "ракетница") { if(shootCount % 5 === 0) { b.push({x:bx, y:by, id:bid, type:"rocket"}); } else { b.push({x:bx, y:by, id:bid, type:"normal"}); } } 
                    else if(curW === "кинжал") { if(shootCount % 10 === 0) { b.push({x:bx, y:by, id:bid, type:"dagger", vy:-2, ric:2}); b.push({x:bx, y:by, id:bid, type:"dagger", vy:0, ric:2}); b.push({x:bx, y:by, id:bid, type:"dagger", vy:2, ric:2}); } else { b.push({x:bx, y:by, id:bid, type:"normal"}); } }
                    else if(curW === "меч" || curW === "когти") { b.push({x:bx, y:by, id:bid, type:"melee", range:60}); }
                    else { b.push({x:bx, y:by, id:bid, type:"normal"}); }
                ...
                function loopScene() { if(!play)return; requestAnimationFrame(loopScene); ctx.clearRect(0,0,650,340); if(t>0)t--;
                    ...
                    if(curW === "дрон") {
                        ...
                        if(dTimer > 180 && en.length > 0) {
                            dTimer = 0; let target = en[0];
                            b.push({x:p1.x, y:p1.y, id:1, type:"drone_bot", tx:target.x, ty:target.y});
                        }
                    }
                    ...
                    b.forEach((x,i)=>{ 
                        if(x.vy) x.y += x.vy;
                        if(x.type === "rocket") {
                            if(en.length > 0) { let target = en[0]; if(target.y > x.y) x.y += 2; if(target.y < x.y) x.y -= 2; }
                            x.x += 6; ctx.font="20px Arial"; ctx.fillText("🚀", x.x, x.y);
                        } 
                        else if(x.type === "drone_bot") {
                            let dx = x.tx - x.x, dy = x.ty - x.y, dist = Math.sqrt(dx*dx + dy*dy);
                            if(dist > 5) { x.x += (dx/dist)*6; x.y += (dy/dist)*6; } else { b.splice(i,1); }
                            ctx.font="18px Arial"; ctx.fillText("🛸", x.x, x.y);
                        }
                        else if(x.type === "dagger") { x.x += 12; if((x.y < 10 || x.y > 330) && x.ric > 0) { x.vy = -x.vy; x.ric--; } ctx.fillStyle = "#ef4444"; ctx.fillRect(x.x, x.y, 8, 4); }
                        else if(x.type === "normal") { x.x+=10; ctx.fillStyle=x.id==1?"#22c55e":"#38bdf8"; ctx.fillRect(x.x,x.y,6,6); }
                        else if(x.type === "melee") { x.x += 15; ctx.strokeStyle = "rgba(239, 68, 68, 0.4)"; ctx.lineWidth = 4; ctx.beginPath(); ctx.arc(x.x, x.y, 20, 0, Math.PI*2); ctx.stroke(); if(x.x > p1.x + x.range) b.splice(i,1); }
                    ...
                    en.forEach((e,i)=>{ e.x-=e.s; ctx.font="20px Arial"; ctx.fillText("🐀",e.x,e.y); 
                        ...
                        if(e.x<p1.x+20&&e.x+20>p1.x&&e.y>p1.y-20&&e.y<p1.y+20){
                            en.splice(i,1); let dmg = 20; let takeDmg = true;
