...
if not st.session_state.play:
    ...
    ch = st.selectbox("Выбери боевого кота:", ["Vasya", "Bulya", "Murka", "Rizyk", "Tomas", "ADMIN"])
    if st.button("⚔️ НАЧАТЬ МАТЧ НА АРЕНЕ", use_container_width=True):
        st.session_state.play, st.session_state.hero = True, ch
        st.rerun()
else:
    if st.button("↩️ СДАТЬСЯ И ВЕРНУТЬСЯ В МЕНЮ", use_container_width=True):
        st.session_state.play = False
        st.rerun()
    ...
    <script>
        ...
        function loop(){{if(!play)return;requestAnimationFrame(loop);ctx.clearRect(0,0,650,320);
            ...
            if(keys["Space"]&&t<=0)shoot();if(t>0)t--;p.x=Math.max(0,Math.min(620,p.x));p.y=Math.max(0,Math.min(290,p.y));ctx.font="25px Arial";
            ctx.fillText(p.e,p.x,p.y); // ИСПРАВЛЕНО: Заменили p.emoji на правильное p.e
